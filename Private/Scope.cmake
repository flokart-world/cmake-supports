...
function (CMS_UNSHIFT_QNAME_PROPERTY _qname)
  if (ARGN)
    ...
    if (_defined)
      ...
      set_property (GLOBAL PROPERTY "${qname}" "${_values}")
    else ()
...
function (CMS_APPEND_TO_QNAME_PROPERTY _qname)
  if (ARGN)
    ...
    if (_defined)
      set_property (GLOBAL APPEND PROPERTY "${_qname}" "${ARGN}")
    else ()
      message (FATAL_ERROR "Property ${_qname} has not been defined.")
    endif ()
  else ()
...
function (CMS_DEFINE_SCOPE_PROPERTY _key)
  get_directory_property (_this CMS::Scope::This)
...
function (CMS_SET_PROPERTY _key _value)
...
function (CMS_UNSHIFT_PROPERTY _key)
  if (ARGN)
    get_directory_property (_this CMS::Scope::This)

    if (_this)
      CMS_UNSHIFT_QNAME_PROPERTY("${_this}::${_key}" "${ARGN}")
    endif ()
  else ()
    message (FATAL_ERROR "at least one argument must be specified.")
  endif ()
...
function (CMS_GET_PROPERTY _var _key)
  ...
  if (_this)
    ...
    if (_defined)
      get_property (_value GLOBAL PROPERTY "${_qname}")
      CMS_RETURN(_var [[${_value}]])
    else ()
      message (FATAL_ERROR "Property ${_qname} has not been defined.")
    endif ()
...
function (CMS_DEFINE_COMMON_PROPERTIES)
  CMS_DEFINE_SCOPE_PROPERTY(CompileOptions)
  CMS_DEFINE_SCOPE_PROPERTY(CompileDefinitions)
  CMS_DEFINE_SCOPE_PROPERTY(ExportName)
  CMS_DEFINE_SCOPE_PROPERTY(IncludeDirectories)
  CMS_DEFINE_SCOPE_PROPERTY(LinkDirectories)
  CMS_DEFINE_SCOPE_PROPERTY(LinkLibraries)
  CMS_DEFINE_SCOPE_PROPERTY(Parent)
  CMS_DEFINE_SCOPE_PROPERTY(ProvidedPackages)
  CMS_DEFINE_SCOPE_PROPERTY(ProvidedTargets)
  CMS_DEFINE_SCOPE_PROPERTY(PublicHeaders)
  CMS_DEFINE_SCOPE_PROPERTY(PublicHeaderDirectories)
  CMS_DEFINE_SCOPE_PROPERTY(RequiredPackages)
  CMS_DEFINE_SCOPE_PROPERTY(RequiredVariables)
  CMS_DEFINE_SCOPE_PROPERTY(SourceGroups)
  CMS_DEFINE_SCOPE_PROPERTY(Type)
  CMS_DEFINE_SCOPE_PROPERTY(Version)
endfunction ()
...
function (CMS_GROUP_FILE _category _relative _absolute)
  ...
  if (NOT _defined)
    CMS_DEFINE_SCOPE_PROPERTY("${_propertyName}")
...
function (CMS_DEFINE_NAMESPACE _name)
  ...
  if (_parent)
    set_directory_properties (PROPERTIES CMS::Scope::This "CMSUser::${_name}")
